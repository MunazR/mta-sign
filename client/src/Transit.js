...
const Transit = () => {
  ...
  const stopNames = [
    ...new Set(
      Object.keys(stations).map((stationId) => stations[stationId]["Stop Name"])
    ),
  ];
      ...
      {Object.keys(stopTimeUpdates)
        ...
        .map((stop) => {
          ...
          const directionLabel =
            direction === "N"
              ? station["North Direction Label"]
              : station["South Direction Label"];

          const routeUpdates = updates
            .filter(
              (update) =>
                update.arrivalTime >= unixNow &&
                update.arrivalTime <= unixNow + 60 * 30
            )
            .reduce((group, update) => {
              ...
              return {
                ...group,
                [routeId]: [...(group[routeId] ?? []), update],
              };
            }, {});

          return (
            <div key={stop}>
              {Object.keys(routeUpdates)
                .sort()
                .map((routeId) => (
                  <TrainArrival
                    ...
                    now={now}
                    arrivalTimes={routeUpdates[routeId].map(
                      (update) => update.arrivalTime
                    )}
                  />
                ))}
            </div>
